...
# Fix ur5 to husky
cid = p.createConstraint(husky, -1, robotID, -1, p.JOINT_FIXED, [0, 0, 0], [0, 0, ], [0., 0., -.2],[0, 0, 0, 1])
...
def executeHelper(actions, goal_file=None):
	...
	while True:
		...
		if action_index >= len(actions):
			...
			if abs(p.getBasePositionAndOrientation(id_lookup[actions[action_index][1]])[0][2] - p.getBasePositionAndOrientation(husky)[0][2]) > 1 and not stick:
			...
			if target == 'door' or target == 'dumpster':
				if not done1:
					x1, y1, o1, done1 = moveTo(x1, y1, o1, [husky, robotID], id_lookup['floor'],tolerances[target],keyboard,speed, 0)
				else:
					x1, y1, o1, done = move(x1, y1, o1, [husky, robotID], [2.9, 4.4, 0],keyboard,speed, 1, 0)
					done1 = not done
			else:
				x1, y1, o1, done = moveTo(x1, y1, o1, [husky, robotID], id_lookup[target],tolerances[target],keyboard,speed, 0)
			...
			if not waiting and not done:
				...
				if actions[action_index][1] == grabbed:
					raise Exception("Cant place object on itself "+actions[action_index][1])
				if actions[action_index][1] in enclosures and isClosed(actions[action_index][1], states, id_lookup):
				...
				if "mop" in grabbed or "sponge" in grabbed or "vacuum" in grabbed:
					cleaner = True
				if "stick" in grabbed:
					stick = True
				...
				cid = constrain(grabbed,actions[action_index][1],cons_link_lookup,cons_pos_lookup,id_lookup,constraints,ur5_dist)
			...
			if not waiting and not done:
				...
				cid = constrain(actions[action_index][1],actions[action_index][2],cons_link_lookup,cons_pos_lookup,id_lookup,constraints,ur5_dist)
